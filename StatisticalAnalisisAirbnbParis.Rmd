...
Ultimamente alrededor de Europa, las personas estan innovando sus destinos, pues la grandes metropolis a pesar de ser muy bonitas, suelen ser muy pobladas y las personas buscan tranquilidad. 


```{r}

#Variables cuantitativas
CalificacionDeLimpiezaBudapest <- budapest_weekdays$cleanliness_rating
PrecioFinalBudapest <- budapest_weekdays$realSum
CapacidadPersonasBudapest <- budapest_weekdays$person_capacity
DistanciaCentroBudapest <- budapest_weekdays$dist

#Variables cualitativas
TipoDeCuartoBudapest <- budapest_weekdays$room_type
CuartoPrivadoBudapest <- budapest_weekdays$room_private
PoseeMasDeUnCuartoBudapest <- budapest_weekdays$multi
FinesDeNegociosBudapest <- budapest_weekdays$biz
```

*PrecioFinal*

Budapest se ha vuelto un destino muy apetecido para turistas europeos, asi que los Airbnb quieren saber el porque y quieren comparar para ver si sus precios son menores a los suyos.
...
```{r}
var.test(PrecioFinalSinAtipicos4,PrecioFinalBudapest, alternative = "greater",conf.level = 0.95)
```

Como podemos ver, el valor p es mayor a 0.05, por lo tanto asumimos varianzas iguales

Ahora, compararemos la media de los precios asumiendo varianzas iguales

$\mu_1$: Media de precios de Airbnb en Paris

$\mu_2$: Media de precios de Airbnb en Budapest
...
```{r}
t.test(PrecioFinalSinAtipicos4, PrecioFinalBudapest,
       alternative = "greater",
       mu = 0, paired = FALSE, var.equal = TRUE,
       conf.level = 0.95)
```

Podemos ver como el valor p es menor que 0.05, por lo que concluimos que la capacidad de personas de Paris es mayor que la de Budapest.
Como podemos ver, nuestro valor p es menor que 0,05 por lo tanto concluimos que el precio de los Airbnb es mayor en Paris que en Budapest.

*Capacidad de personas*

Como se dijo anteriormente, Budapest ultimamente ha sido un destino muy apetecido por los turistas, es por esto que la empresa Airbnb quiere saber si se puede tratar de la capacidad de personas que ofrece la ciudad de Budapest con respecto a Paris, que por muchos años ha sido uno de los destinos mas importantes para la empresa.

A continuacion se planteara una hipotesis para dos variables para comprobar si la capacidad media de personas en Paris es mayor que la capacidad media de personas en Budapest. Primero vamos a verificar si podemos asumir varianzas iguales o diferentes:

$\sigma_1^2$: Varianza de capacidad de personas en Airbnb de Paris

$\sigma_2^2$: Varianza de capacidad de personas en Airbnb de Budapest
...
```{r}
var.test(CapacidadPersonas,CapacidadPersonasBudapest, alternative = "greater",conf.level = 0.95)
```

Podemos ver como el valor $p$ es mayor que 0.05, por lo que podemos asumir varianzas iguales.

Ahora, compararemos la capacidad media de personas asumiendo varianzas iguales

$\mu_1$: capacidad media de personas de Airbnb en Paris

$\mu_2$: capacidad media de personas en Budapest
...
```{r}
t.test(CapacidadPersonas, CapacidadPersonasBudapest,
       alternative = "greater",
       mu = 0, paired = FALSE, var.equal = TRUE,
       conf.level = 0.95)
```
...
La ciudad de Budapest ultimamente ha sido un destino muy aclamado en general, Airbnb lo sabe por lo cual quiere saber si esto se debe a la distancia hacia el centro que tiene Budapest en comparacion a Paris. Debido a que esta ciudad en general sea centro, sur o norte siempre es muy apetecida.

Es por esto que se propuso una hipotesis para dos variables para comprobar si la distancia al centro en Paris es mayor que la distancia al centro en Budapest. El primer paso es revisar si se puede asumir varianzas iguales o diferentes con un nivel del confianza del 95%:

$\sigma_1^2$: Varianza de Distancia al centro en Airbnb de Paris

$\sigma_2^2$: Varianza de Distancia al centro en Airbnb de Budapest


$H_0: \sigma_1^2 = \sigma_2^2$ Igual 

$H_1: \sigma_1^2 \neq \sigma_2^2$ Diferente



```{r}
var.test(DistanciaCentro,DistanciaCentroBudapest, alternative = "greater",conf.level = 0.95)
```

El valor p es mayor que el nivel de significancia al 95% (osea 0.05), a partir de esto se interpreta que las varianzas son iguales. Por ende, se puede comparar la distancia al centro de ambos destinos con varianzas iguales. Al igual con un nivel de confianza del 95%:

$\mu_1$: Distancia al centro de Airbnb en Paris

$\mu_2$: Distancia al centro de Airbnb en Budapest
...
```{r}
t.test(DistanciaCentro, DistanciaCentroBudapest,
       alternative = "greater",
       mu = 0, paired = FALSE, var.equal = TRUE,
       conf.level = 0.95)
```
El valor p es 0.82, es mayor a 0.05 por ende no hay suficiente evidencia para rechazar la hipotesis nula. 
