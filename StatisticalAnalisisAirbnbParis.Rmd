...
Ultimamente alrededor de Europa, las personas estan inovando sus destinos, pues la grandes metropolis a pesar de ser muy bonitas, suelen ser muy pobladas, y las personas buscan tranquilidad. 
...
Budapest se ha vuelto un destino muy apetecido los turistas europeos, asi que los Airbnb quieren saber el porque y quieren comparar para ver si sus precios son menores a los suyos.
...
como podemos ver, nuestro valor p es menor que 0,05 por lo tanto concluimos que el precio de los rbnb es mayor en paris que en Budapest.
...
$\mu_1$: capacidad media de personas de rbnb en paris
...
```{r}
boxplot(PrecioFinal~TipoDeCuarto, data = Paris_weekdays, ylim = c(0,1500) )
```
...
En esta sección del informe se va a aplicar los tests de chi cuadrado, en la que se evaluará si existe una relación significativa entre dos variables categóricas. Estos tests se van a aplicar tanto a las variables cuantitativas como a las cualitativas de nuestro estudio:
