...
-   **Objetivo 6:** Realizar graficas que ilustren la posible relacion entre 2 variables del estudio, como pueden ser graficos de barras apliadas para las variables cualitativas.

-   **Objetivo 7:** Realizar hipotesis para a la diferencia de medias en muestras independientes y /o dependientes.

- **Objetivo 8:** Conocer si existen diferencias entre la distrubicion de las medias para variables cuantitativas mediante la prueba de anova.

- **Objetivo 9:** Conocer si existen diferencias en la proporcion para variables cualitativas mediante la prueba de Ji-Cuadrado

-  **Objetivo 10:** Plantear una relacion lineal para nuestras variables y ver como se comporta este modelo

-  **Objetivo 11:** Validar los supuestos de regresion del modelo de regresion lineal
...
La base de datos de la cual vamos a explorar está dentro de un formato .csv y, para una mayor practicidad, vamos a guardar cada una de las variables a explorar dentro de un elemento distinto de nuestro entorno, tal como se muestra a continuación:
...
## **Estadística descriptiva para las Variables cuantitativas**

### **Calificación de limpieza**

Antes de realizar el analisis de esta variable, vamos a realizar un filtro de valores atipicos, este se vera representado con los siguientes graficos:
...
```
Vemos que el valor de este coeficiente es positivo, podemos decir que esta variable tiene una asimetria hacia la derecha.
*tabla de frecuencia*
```{r}
...
Se graficara la variable para ver si tiene valores atipicos:
...
tabla <- as.data.frame.table(tablaDeFrecuencia)
frec_rel <- prop.table(tablaDeFrecuencia)
frec_rel_acum <- cumsum(frec_rel)

tabla <- as.data.frame.table(tablaDeFrecuencia)
tabla$frec_rel <- frec_rel
tabla$frec_rel_acum <- frec_rel_acum
tabla <- tabla[,c(1, 2, 3, 4)]
colnames(tabla) <- c("categoria", "frec. abs", "frec. rel", "frec. rel. acum")
rownames(tabla) <- NULL
...
tabla <- table(CuartoPrivado)
pie(tabla, main="Grafico de pastel Cuarto privado", labels = c("Sin cuarto privado", "Con cuarto privado"), col = colores)
```
...
tabla <- as.data.frame.table(frecuencia_FinNegocios)
frec_rel <- prop.table(frecuencia_FinNegocios)
frec_rel_acum <- cumsum(frec_rel)
...
$$[384.90~; ~412.69]$$ Es decir, que el precio promedio de los Airbnb en París entre semana, con un 95% de confianza, está dentro del rango 384,90 y 412,69 Euros.
...
$$[2.90~; ~2.99]$$ Es decir, que la capacidad promedio de personas de los AirBnB en París entre semana, con un 95% de confianza, está dentro del rango de 2,9 a 2,99 personas.
...
$$[2.94~; ~3.07]$$ Es decir, que la distancia promedio al centro de los Airbnb en París entre semana, con un 98% de confianza, está dentro del rango de 2.94 kilómetros a 3.07 kilómetros.
...
Dentro de esta sección del estudio vamos a plantear hipótesis a las variables, ya sea respecto a la media o a la proporción, dependiendo de su caracteristica cualitativa o cuantitiva. Para realizar esto vamos a utilizar como base para plantear nuestras hipotesis los datos de AirBnb en Roma durante los dias entresemana, a partir de esto vamos a comprobar si se cumplen dichas hipotesis dentro de los Airbnb de Paris durante los dias entre semana.

Importamos los datos de AirBnb Roma entre semana:
...
Los alojamientos de Airbnb Paris son las mas visitadas en el mundo, pues Francia es un pais bastante turistico, y ademas Paris es una de las ciudades que mas turistas recibe en el mundo, debido a esto quieren dar la mejor atencion a sus turistas, pues gran parte vive de estos. Ahora bien, no a todos les alcanza el dinero y les toca compartir, pero ellos desean saber cual es la proporcion de personas que tienen cuarto privado, por esto van a comparar con los Airbnb en Roma:
...
Para esto ejecutamos el siguiente codigo para probar las hipotesis:
...
Dentro de esta seccion se van a comparar tanto la media o la proporcion de 2 variables cruzadas dentro de nuestro estudio para poder sacar conclusiones sobre estas:
$$$$

#### **Precios del AirBnb de acuerdo al si el cuarto es compartido**

Dentro de este estudio queremos saber como varian sus precios los AirBnb de acuerdo a si el cuarto es compartido o no, por lo que vamos a plantear las siguientes hipotesis:
...
Para comprobar esto primero debemos plantear hipotesis de varianzas y poder ver si podemos tomar varianzas iguales o diferentes, por lo que vamos a plantear las siguientes hipotesis:
...
* $\sigma_1^2$: Varianza de precio de Airbnb en paris con cuarto compartido

* $\sigma_2^2$: Varianza de precio de Airbnb en paris con cuarto no compartido
...
Como podemos ver, el valor p es mucho menor que 0.05 por lo tanto podemos **asumir varianzas diferentes**. Ahora, compararemos la media de los precios asumiendo varianzas diferentes:
...
#### **Capacidad de personas cuando sus habitaciones son privadas o compartidas**

Se desea conocer que relacion tiene media la capacidad de personas cuando las habitaciones cuando son privadas o publicas, es por esto que se plantean las siguientes hipotesis:
...
Podemos ver como el valor $p$ es mucho menor que 0.05, por lo que podemos asumir varianzas varianzas diferentes. Ahora, compararemos la capacidad media de personas asumiendo varianzas diferentes:
...
Aqui podemos ver que el valor p es mucho mayor a 0,05, por lo tanto podemos concluir que no existe evidencia para decir que la media de personas con cuarto privado tenga menos capacidad de personas que los cuartos compartido.
...
A continuación se muestra una grafica de barras apiladas para mostrar la relación que tienen las variables de cuarto privado y posee mas de un cuarto
...
Para AirBnb resulta importante poder comparar los diferentes precios que hay de acuerdo con el tipo de alojamiento. Teniendo en cuenta que el tipo de cuarto tiene 3 posibles valores que son : Apartamento/Casa entera, cuarto privado y cuarto compartido, se van a plantear si el precio en cada uno de estos grupos es igual:


$$H_0: \mu_{Completo} =  \mu_{Privado} = \mu_{Compartido}$$

$$H_1: Al~menos~una~de~ellas~es~diferente$$

Antes de realizar el codigo de anova podemos analizar el comportamiento de los grupos en forma de diagramas de cajas y bigotes:

```{r}
baseSinAtipicos <- filter(Paris_weekdays, Paris_weekdays$realSum < 800)
boxplot(baseSinAtipicos$realSum~baseSinAtipicos$room_type, data = baseSinAtipicos)
PrecioFinalSinAtipicos  <- baseSinAtipicos$realSum
TipoDeCuartoSinAtipicos <- baseSinAtipicos$room_type
```

Despues de haber hecho esto si podemos ejecutar el codigo del anova:

*Ejecutamos el codigo de anova:*

```{r}
anova<-aov(lm(PrecioFinalSinAtipicos~TipoDeCuartoSinAtipicos))
summary(anova)
```
...
#### **Compracion del puntaje de limpieza teniendo en cuenta su tipo de alojamiento**

Para AirBnb es sumamente importante mirar si hay algun tipo de diferencia en el puntaje de limpieza de acuerdo a las categorias del tipo de alojamiento. Teniendo en cuenta que el tipo de cuarto tiene 3 posibles valores que son : Apartamento/Casa entera, cuarto privado y cuarto compartido, se van a plantear si el precio en cada uno de estos grupos es igual: 

$$H_0: \mu_{Completo} =  \mu_{Privado} = \mu_{Compartido}$$

$$H_1: Al~menos~una~de~ellas~es~diferente$$

Antes de ejecutar el codigo de anova podemos mirar como ser verian los diagramas de cajas y bigotes de estos 3 grupos:

```{r}
boxplot(CalificacionDeLimpieza~TipoDeCuarto)
```
...
$$Ho: No~existe~relacion~entre~si~es~por~fin~de~negocio~y~si~posee~mas~de~un~cuarto.$$
...
```
El valor p es muy pequeño por lo que se concluye que si hay una relacion

```{r, warning=FALSE}
tabla <- crosstab(FinesDeNegocios,PoseeMasDeUnCuarto,prop.c = TRUE,plot = FALSE)
tabla
...
$$Ho: No~existe~relacion~entre~si~el~cuarto~es~privado~y~si~posee~mas~de~un~cuarto.$$

$$H1: Existe~relacion~entre~si~es~para~cuarto~privado~y~si~posee~mas~de~un~cuarto.$$
...
Aqui podemos ver el valor p el cual vuelve a ser muy pequeño (menor a 0.05), entonces podemos concluir que si existe una relacion entre cuarto privado y si posee mas de un cuarto. 
...
### **Regresion lineal - Distancia al centro vs Precio final**

Dentro de esta seccion del informe se va a plantear una regresion lineal entre las varaibles de distancia al centro de la ciudad, siendo esta la variable independiente y el precio final del alojamiento que seria la variable dependiente. Sin embargo cuando graficamos estas 2 variables en una grafico de dispersion tenemos el siguiente resultado:

```{r}
#names(Paris_weekdays)
baseSinAtipicos <- filter(Paris_weekdays, Paris_weekdays$realSum < 1500)

plot(baseSinAtipicos$dist, baseSinAtipicos$realSum)

```

Aqui podemos ver que los datos estan muy dispersos y no siguen alguna tendencia que se pueda observar a simple vista. Esto se debe a que realmente el precio de un alojamiento en AirBnb no depende exclusivamente de la distancia al centro de la ciudad, sino tambien por otros atributos como si es compartido o privado, los metros cuadrados que tiene o sus acabados. Es por esto que para este caso vamos a fijar una de las variables que intervienen en nuestro modelo que seria el del tipo de alojamiento y lo vamos a fijar como "Cuarto compartido". A continuacion se ve el grafico de dispersion de esto:

```{r}
baseSinAtipicos1 <-  filter(baseSinAtipicos, baseSinAtipicos$room_type == "Shared room")

plot(baseSinAtipicos1$dist, baseSinAtipicos1$realSum)
```
...
Queremos probar si hay una relacion entre la distancia del alojamiento y el precio , por lo que se hacen la siguientes hipotesis:

$$H_0: \rho = 0$$
$$H_1: \rho \neq 0$$

```{r}
cor.test(baseSinAtipicos1$dist, baseSinAtipicos1$realSum)
```

Con esto podemos ver que existe una relacion entre la distancia del alojamiento y el precio dado que es un cuarto privado. Ademas, vemos que tienen una relacion debil negativa puesto que el coeficiente de relacion entre las dos variables es de: $r = -0.38689$. Es decir que entre mas lejano sea el AirBnb del centro de la ciudad menor va a ser el precio

##### **Prueba de linealidad**

A continuacion hacemos la prueba de linealidad en nuestro modelo:

$$H_0: \beta_1 = 0$$
$$H_1: \beta_1 \neq 0$$

```{r}
Regresion <- lm(baseSinAtipicos1$realSum~dist, data = baseSinAtipicos1)
summary(Regresion)
```

Vemos que nuestro valor p es bastante menor que nuestro nivel de significancia ya que $0.00722 < 0.05$, por lo que se rechaza $H_0$ y podemos decir que si existe una relacion lineal entre las variables. 

El modelo de regresion que se ajusta a este caso es el siguiente:

$$\hat{Y} = \beta_0 + \beta_1X $$
$$\hat{Y} = 200.5 -18.2X $$

Vemos que en esta ecucion el intersecto es 7197 y la pendiente es -18.2. Es decir que por cada 1km que se separe del centro el precio del AirBnb se reduce 18.2 Euros.

#### **Supuestos de la regresion (Residuos)**

Con el siguiente codigo sacamos todos los residuos de nuestro modelo de regresion:
...
A continuacion se va a realizar la prueba de normalidad a los residuos de la regresion usando la prueba de Shapiro ya que se utilizaron mas de 50 datos. Para esto se plantean las siguiente hipotesis:


$$H_0: Los~residuos~se~distribuyen~de~forma~normal$$
$$H_1: Los~residuos~no~se~distribuyen~de~forma~normal$$
...
Según el resultado del test de Shapiro, el valor p obtenido es menor que el nivel de significancia establecido (alfa), lo que indica que debemos rechazar la hipótesis nula ($H_0$). En consecuencia, podemos concluir que los residuos no siguen una distribución normal.

No obstante, podemos considerar la asunción de normalidad en los residuos debido al **Teorema Central del Límite**, ya que se ha utilizado un tamaño de muestra mayor a 30.

##### **Supuesto de homogeneidad**

Para esta prueba vamos a usar el test de Breush Pagan y vamos a plantear las siguientes hipotesis:

$$H_0: Los~residuos~presentan~varianzas~constante.$$
$$H_1: Los~residuos~no~presentan~varianzas~constante.$$
```{r, echo = FALSE}
baseSinAtipicos <- filter(Paris_weekdays, Paris_weekdays$realSum < 400)
baseSinAtipicos1 <-  filter(baseSinAtipicos, baseSinAtipicos$room_type == "Shared room")
Regresion <- lm(baseSinAtipicos1$realSum~dist, data = baseSinAtipicos1)
```
...
Vemos que el valor p ($0.44$) es mayor que nuestro alfa, por lo que no podemos rechazar la hipotesis nula y por lo tanto podemos decir que los residuos si tienen varianzas constantes.

#### **Supuesto de independencia**

En este caso no podemos aplicar la prueba de independencia de los residuos puesto que el analisis entre la distancia y el precio final no tiene series de tiempo.

#### **Intervalos de confianza**

A continuacion se muestran los intervalos de confianza del modelo de regresion con un intervalo entre 0 km y 6km con saltos de 0.1 km:
```{r}
rango <- seq(0, 6, by = 0.1)
nuevos_valores <- data.frame(dist = rango)
intervalos_confianza <- predict(Regresion, newdata = nuevos_valores, interval = "confidence", level = 0.95)
...
A continuacion se muestran los intervalos de prediccion del modelo de regresion con un intervalo entre 0 km y 6km con saltos de 0.1 km:

```{r}
predicciones <- predict(Regresion, newdata = nuevos_valores, interval = "prediction", level = 0.95)
predicciones
```
...
- No existe alguna evidencia que me muestre que los cuartos no compartidos sean mas caros que los cuartos compartidos.

- como bien sabemos existen tres tipos de cuartos, compartido, privado y completo, en esta caso podemos ver que existen diferencias en las medias y segun la prueba hecha despues de hacer el analisis para ver notar sus diferentes medias obtenemos que 
$\mu_{Completo} >  \mu_{Privado} > \mu_{Compartido}$

- podemos que el tipo de cuarto influye es si este es para negocio o no, pues podemos ver que cuando este visita es con fines de negocios se decantan mas por cuartos compartidos

- podemos ver que existe una relacion lineal inversa entre la distancia al centro y el precio

- aunque nuestra prueba shapiro nos indique que no encontro que estos residuos se distribuyan normal, nosotros lo asumimos pues tenemos mas de 3000 datos, los cuales son mas que suficientes para considerar normalidad.
